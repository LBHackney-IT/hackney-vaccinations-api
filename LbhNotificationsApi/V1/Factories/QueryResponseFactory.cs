        ...
        public static List<Notification> ToNotification(this QueryResponse response)
        {
            ...
            foreach (Dictionary<string, AttributeValue> item in response.Items)
            {
                notifications.Add(new Notification
                {
                    ...
                    RequireAction = item.ContainsKey("require_action") && item["require_action"].S != null ? bool.Parse(item["require_action"].S) : false,
                    ...
                    IsReadStatus = item.ContainsKey("is_read_status") && item["is_read_status"].S != null ? bool.Parse(item["is_read_status"].S) : false,
                    RequireEmailNotification = item.ContainsKey("require_email_notification") && item["require_email_notification"].S != null ? bool.Parse(item["require_email_notification"].S) : false,
                    RequireLetter = item.ContainsKey("require_letter") && item["require_letter"].S != null ? bool.Parse(item["require_letter"].S) : false,
                    RequireSmsNotification = item.ContainsKey("require_sms_notification") && item["require_sms_notification"].S != null ? bool.Parse(item["require_sms_notification"].S) : false,
